...
from .commons import spacy_tokenizer, ARXIV_CHECKPOINTS, Corrector, spell_tokenizer
...
class CorrectorLstmLstm(Corrector):
    ...
    def correct_strings(self, mystrings: List[str], return_all=False) -> List[str]:
        self.__model_status()
        if self.tokenize:
            mystrings_tokenized = [spell_tokenizer(my_str) for my_str in mystrings]
        ...
        if return_all:
            return mystrings_tokenized, return_strings
        else:
